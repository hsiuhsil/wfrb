...
if BURST:
    ...
    DATAROOT = "/scratch2/p/pen/hsiuhsil/burst_candidates/56555_1hr_0014/raw_data/"
    OUT_ROOT = '/scratch2/p/pen/hsiuhsil/burst_candidates/56555_1hr_0014/dm0'
    FILENAME = "guppi_56555_wigglez1hr_centre_0014_0003.fits"

    # Calibration source scans.
    SRCFILES = [
        "guppi_56555_3C147_0006_0001.fits",
        "guppi_56555_3C147_0007_0001.fits",
        "guppi_56555_3C147_0008_0001.fits",
        "guppi_56555_3C147_0009_0001.fits",
        ]

    # Hard code the phase of the noise cal, since I'm too lazy to write an algorithm
    # to find it.
    DATA_CAL_PHASE = 40
    SRC_CAL_PHASES = [57, 43, 12, 45]

    # The small time slice of data containing the burst.
    TSL = np.s_[23000:57000]
...
def reformat_raw_data():
    ...
    if BURST:
        data, time, freq, ra, dec, az, el = read_fits_data(hdulist, 50, 80)
    else:
...
def fit_basic():
    ...
    pars0 = [0.063, 0., 0.001, 2., 0.001, 0.001]
...
#    plot_pulse(initial_model, freq, time, pars0_real[0], pars0_real[1])
    print np.mean(data_I, axis=0)
    plt.figure(1)
    plt.plot(time,np.mean(data_I, axis=0))
    plt.title('np.mean(data_I, axis=0)' , fontsize=24)

    plt.figure(2)
    plt.plot(time, data_I[800,:])
    plt.title('data_I[800,:]', fontsize=24)

    plt.figure(3)
    plt.plot(time, data_I[2000,:])
    plt.title('data_I[2000,:]', fontsize=24)

    plt.figure(4)
    plt.plot(time, data_I[3000,:])
    plt.title('data_I[3000,:]', fontsize=24)

    plt.figure(5)
    plt.plot(time, np.mean(data_I[:2048,:],axis=0))
    plt.title('data_I[:2048,:]', fontsize=24)
...
#    plot_pulse(data_I[800,:], freq, time, pars0_real[0], pars0_real[1])
    plt.show()
